            ...
            <>
              {showForm && (
                <div className="card-vintage p-6 mb-6">
                  ...
                  <label className="flex items-center gap-2 mb-4 cursor-pointer"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} style={{ accentColor: "oklch(34.6% 0.074 256.1)" }} /><span style={{ fontFamily: "'EB Garamond', serif", fontSize: "0.9rem", color: "oklch(47.2% 0.088 247.4)" }}>Active (visible to verified users)</span></label>
              ...
              {contentLoading ? <div className="text-center py-12" style={{ fontFamily: "'EB Garamond', serif", color: "oklch(74.2% 0.118 90.2)" }}>Loading...</div> : content && content.length > 0 ? (
                  ...
                  {content.map((item: any) => (
                    <div key={item.id} className="card-vintage p-4 flex items-center justify-between">
                      <div>
                        ...
                        <p style={{ fontFamily: "'EB Garamond', serif", fontSize: "0.8rem", color: "oklch(74.2% 0.118 90.2)", margin: 0 }}>{item.category.replace("_", " ")} {item.gradeLevel ? `· Grades ${item.gradeLevel}` : ""}</p>
                      </div>
              ...
              ) : <div className="text-center py-16"><BookOpen size={48} style={{ color: "oklch(87.6% 0.068 89.7)", margin: "0 auto 1rem" }} /><p style={{ fontFamily: "'EB Garamond', serif", color: "oklch(74.2% 0.118 90.2)", fontStyle: "italic" }}>No content yet. Add your first educational resource.</p></div>}
            </>
          )}

          {activeTab === "requests" && (
            <>
              {requestsLoading ? <div className="text-center py-12" style={{ fontFamily: "'EB Garamond', serif", color: "oklch(74.2% 0.118 90.2)" }}>Loading...</div> : requests && requests.length > 0 ? (
                  ...
                  {requests.map((req: any) => (
                    <div key={req.id} className="card-vintage p-4 flex items-center justify-between">
                      <div>
                        ...
                        <p style={{ fontFamily: "'EB Garamond', serif", fontSize: "0.8rem", color: "oklch(74.2% 0.118 90.2)", margin: 0 }}>{req.email} · {req.role || "—"} {req.organization ? `· ${req.organization}` : ""}</p>
                        {req.reason && <p style={{ fontFamily: "'EB Garamond', serif", fontSize: "0.8rem", color: "oklch(47.2% 0.088 247.4)", margin: "0.25rem 0 0", fontStyle: "italic" }}>"{req.reason}"</p>}
                      </div>
                      <div className="flex items-center gap-2">
                        <span style={{ padding: "0.2rem 0.6rem", background: req.approved ? "oklch(85% 0.08 155)" : req.verified ? "oklch(87.6% 0.068 89.7)" : "oklch(90% 0.06 75)", color: req.approved ? "oklch(25% 0.08 155)" : "oklch(47.2% 0.088 247.4)", fontSize: "0.7rem", fontFamily: "'Playfair Display', serif" }}>
              ...
              ) : <div className="text-center py-16"><p style={{ fontFamily: "'EB Garamond', serif", color: "oklch(74.2% 0.118 90.2)", fontStyle: "italic" }}>No access requests yet.</p></div>}
            </>
          )}
        </div>
      </div>
