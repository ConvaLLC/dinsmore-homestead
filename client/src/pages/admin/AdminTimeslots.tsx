...
function StatusBadge({ slot }: { slot: Slot }) {
  ...
  if (!slot.active) return <span style={{ display:"inline-flex",alignItems:"center",gap:"4px",padding:"2px 8px",borderRadius:"2px",fontSize:"0.7rem",fontFamily:"'Playfair Display',serif",letterSpacing:"0.06em",background:"oklch(87% 0.032 72)",color:"oklch(46% 0.06 56)" }}><XCircle size={10}/> Inactive</span>;
...
function CapacityBar({ slot }: { slot: Slot }) {
  ...
  return (
    <div style={{ display:"flex",alignItems:"center",gap:"8px" }}>
      <div style={{ flex:1,height:"6px",background:"oklch(87% 0.032 72)",borderRadius:"3px",overflow:"hidden" }}>
        <div style={{ width:`${pct}%`,height:"100%",background:color,transition:"width 0.3s ease" }}/>
      </div>
      <span style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.8rem",color:"oklch(46% 0.06 56)",whiteSpace:"nowrap" }}>{slot.ticketsSold}/{slot.capacity}</span>
    </div>
  );
}
...
function EditSlotModal({ slot, onClose, onSave, isSaving }: { slot: Slot; onClose: () => void; onSave: (f: EditForm) => void; isSaving: boolean; }) {
  ...
  const inp: React.CSSProperties = { width:"100%",padding:"0.45rem 0.65rem",border:"1px solid oklch(72% 0.05 62)",background:"oklch(93% 0.025 75)",fontFamily:"'EB Garamond',serif",fontSize:"0.95rem",color:"oklch(22% 0.04 50)",outline:"none" };
  const lbl: React.CSSProperties = { fontFamily:"'Playfair Display',serif",fontSize:"0.68rem",letterSpacing:"0.1em",textTransform:"uppercase",color:"oklch(46% 0.06 56)",display:"block",marginBottom:"0.25rem" };
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"oklch(0% 0 0 / 0.55)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div style={{ background:"oklch(96% 0.018 80)",border:"1px solid oklch(72% 0.05 62)",borderTop:"4px solid oklch(55% 0.11 72)",padding:"2rem",width:"100%",maxWidth:"420px",boxShadow:"0 20px 60px oklch(0% 0 0 / 0.3)" }}>
        <h3 style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.1rem",marginBottom:"1.25rem",color:"oklch(22% 0.04 50)" }}>Edit Slot &mdash; {formatDate(slot.slotDate)} {slot.startTime}</h3>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"1.5rem" }}>
          ...
          <label htmlFor="slot-active" style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.9rem",color:"oklch(30% 0.05 52)",cursor:"pointer" }}>Slot is active (visible to public)</label>
        </div>
        {slot.ticketsSold > 0 && (
...
export default function AdminTimeslots() {
  ...
  const ss: React.CSSProperties = { background:"oklch(96% 0.018 80)",border:"1px solid oklch(82% 0.04 65)",borderTop:"3px solid oklch(55% 0.11 72)",padding:"1.5rem",marginBottom:"1.5rem" };
  const inp: React.CSSProperties = { padding:"0.45rem 0.65rem",border:"1px solid oklch(72% 0.05 62)",background:"oklch(93% 0.025 75)",fontFamily:"'EB Garamond',serif",fontSize:"0.9rem",color:"oklch(22% 0.04 50)",outline:"none",width:"100%" };
  const lbl: React.CSSProperties = { fontFamily:"'Playfair Display',serif",fontSize:"0.65rem",letterSpacing:"0.1em",textTransform:"uppercase",color:"oklch(46% 0.06 56)",display:"block",marginBottom:"0.2rem" };
      ...
      <div className="container py-8" style={{ maxWidth:"1100px" }}>
        <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"1.5rem",flexWrap:"wrap",gap:"1rem" }}>
          <div>
            ...
            <h1 style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.6rem",color:"oklch(22% 0.04 50)",margin:0 }}>Tour Time Slots</h1>
            {currentEvent && <p style={{ fontFamily:"'EB Garamond',serif",fontSize:"1rem",color:"oklch(46% 0.06 56)",marginTop:"0.25rem" }}>{(currentEvent as any).title}</p>}
          </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:"1rem",marginBottom:"1.5rem" }}>
          {([{label:"Active Slots",value:totalSlots,icon:<Clock size={18}/>,warn:false},{label:"Total Capacity",value:totalCapacity,icon:<Users size={18}/>,warn:false},{label:"Tickets Sold",value:totalSold,icon:<CheckCircle size={18}/>,warn:false},{label:"Sold Out Slots",value:soldOutSlots,icon:<AlertTriangle size={18}/>,warn:soldOutSlots>0}] as const).map(stat => (
            <div key={stat.label} style={{ background:"oklch(96% 0.018 80)",border:`1px solid ${stat.warn?"oklch(55% 0.18 25)":"oklch(82% 0.04 65)"}`,padding:"1rem",textAlign:"center" }}>
              <div style={{ color:stat.warn?"oklch(45% 0.18 25)":"oklch(55% 0.11 72)",marginBottom:"0.4rem",display:"flex",justifyContent:"center" }}>{stat.icon}</div>
              <div style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.6rem",color:"oklch(22% 0.04 50)",lineHeight:1 }}>{stat.value}</div>
              <div style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.8rem",color:"oklch(46% 0.06 56)",marginTop:"0.2rem" }}>{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        {showBulkForm && (
          <div style={ss}>
            <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.1rem",marginBottom:"1.25rem",display:"flex",alignItems:"center",gap:"0.5rem" }}><Zap size={16} style={{ color:"oklch(55% 0.11 72)" }}/> Bulk Generate Tour Slots</h2>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"1.25rem" }}>
              ...
              <label htmlFor="replace-existing" style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.9rem",color:"oklch(30% 0.05 52)",cursor:"pointer" }}>Replace existing slots in this date range</label>
            </div>
            <div style={{ display:"flex",gap:"0.75rem" }}>
        ...
        {showSingleForm && (
          <div style={ss}>
            <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.1rem",marginBottom:"1.25rem",display:"flex",alignItems:"center",gap:"0.5rem" }}><Plus size={16} style={{ color:"oklch(55% 0.11 72)" }}/> Add Single Time Slot</h2>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"1rem",marginBottom:"1rem",flexWrap:"wrap" }}>
          <div style={{ display:"flex",alignItems:"center",gap:"0.5rem" }}>
            <Filter size={14} style={{ color:"oklch(55% 0.11 72)" }}/>
          ...
          <span style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.85rem",color:"oklch(55% 0.11 72)",marginLeft:"auto" }}>{filteredSlots.length} slot{filteredSlots.length!==1?"s":""} shown</span>
        </div>

        {slotsLoading ? (
          <div style={{ textAlign:"center",padding:"3rem",fontFamily:"'EB Garamond',serif",color:"oklch(55% 0.11 72)" }}>Loading slots...</div>
        ) : sortedDates.length === 0 ? (
          <div style={{ ...ss,textAlign:"center",padding:"3rem" }}>
            <Calendar size={40} style={{ color:"oklch(72% 0.05 62)",margin:"0 auto 1rem" }}/>
            <p style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.1rem",color:"oklch(46% 0.06 56)" }}>No time slots yet</p>
          ...
          <div>
            ...
            {sortedDates.map(dateKey => {
              ...
              return (
                <div key={dateKey} style={{ marginBottom:"0.75rem",border:"1px solid oklch(82% 0.04 65)",background:"oklch(96% 0.018 80)" }}>
                  <button onClick={() => toggleDate(dateKey)} style={{ width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0.75rem 1rem",background:"transparent",border:"none",cursor:"pointer",borderBottom:isExpanded?"1px solid oklch(82% 0.04 65)":"none" }}>
                    <div style={{ display:"flex",alignItems:"center",gap:"1rem" }}>
                      <span style={{ fontFamily:"'Playfair Display',serif",fontSize:"0.95rem",color:"oklch(22% 0.04 50)",fontWeight:600 }}>{formatDate(dateKey+"T12:00:00")}</span>
                      <span style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.8rem",color:"oklch(55% 0.11 72)" }}>{daySlots.length} slot{daySlots.length!==1?"s":""}</span>
                    ...
                    <div style={{ display:"flex",alignItems:"center",gap:"1.5rem" }}>
                      <span style={{ fontFamily:"'EB Garamond',serif",fontSize:"0.85rem",color:dayRemaining===0?"oklch(40% 0.18 25)":"oklch(46% 0.06 56)" }}>{daySold}/{dayCapacity} sold &middot; {dayRemaining} remaining</span>
                      {isExpanded ? <ChevronUp size={16} style={{ color:"oklch(55% 0.11 72)" }}/> : <ChevronDown size={16} style={{ color:"oklch(55% 0.11 72)" }}/>}
                    </div>
                  </button>
                  {isExpanded && (
                        ...
                        <thead>
                          <tr style={{ background:"oklch(90% 0.025 72)" }}>
                            {["Time","Duration","Capacity","Sold","Remaining","Price","Status","Actions"].map(h => (
                              <th key={h} style={{ padding:"0.5rem 0.75rem",textAlign:"left",fontFamily:"'Playfair Display',serif",fontSize:"0.65rem",letterSpacing:"0.1em",textTransform:"uppercase",color:"oklch(46% 0.06 56)",fontWeight:600,borderBottom:"1px solid oklch(82% 0.04 65)" }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {daySlots.map((slot,idx) => {
                            ...
                            return (
                              <tr key={slot.id} style={{ background:idx%2===0?"transparent":"oklch(93% 0.02 75)",opacity:slot.active?1:0.6 }}>
                                <td style={{ padding:"0.6rem 0.75rem",fontFamily:"'Playfair Display',serif",fontSize:"0.85rem",color:"oklch(22% 0.04 50)",whiteSpace:"nowrap" }}>{slot.startTime}</td>
                                <td style={{ padding:"0.6rem 0.75rem",fontFamily:"'EB Garamond',serif",fontSize:"0.8rem",color:"oklch(46% 0.06 56)",whiteSpace:"nowrap" }}>{slot.endTime ? `to ${slot.endTime}` : "—"}</td>
                                <td style={{ padding:"0.6rem 0.75rem",minWidth:"140px" }}><CapacityBar slot={slot}/></td>
                                <td style={{ padding:"0.6rem 0.75rem",fontFamily:"'EB Garamond',serif",fontSize:"0.85rem",textAlign:"center",color:"oklch(30% 0.05 52)" }}>{slot.ticketsSold}</td>
                                <td style={{ padding:"0.6rem 0.75rem",fontFamily:"'EB Garamond',serif",fontSize:"0.85rem",textAlign:"center",color:remaining===0?"oklch(40% 0.18 25)":"oklch(30% 0.05 52)",fontWeight:remaining===0?700:400 }}>{remaining===0?"FULL":remaining}</td>
                                <td style={{ padding:"0.6rem 0.75rem",fontFamily:"'EB Garamond',serif",fontSize:"0.85rem",color:"oklch(46% 0.06 56)" }}>{slot.price?`$${parseFloat(slot.price).toFixed(2)}`:"Event price"}</td>
                                <td style={{ padding:"0.6rem 0.75rem" }}><StatusBadge slot={slot}/></td>
                                <td style={{ padding:"0.6rem 0.75rem" }}>
                                  <div style={{ display:"flex",gap:"0.4rem" }}>
                                    <button onClick={() => setEditingSlot(slot)} title="Edit" style={{ background:"transparent",border:"1px solid oklch(72% 0.05 62)",padding:"4px 8px",cursor:"pointer",color:"oklch(46% 0.06 56)",borderRadius:"2px" }}><Edit2 size={12}/></button>
