    ...
    <div>
      ...
      <div className="py-8" style={{ background: "oklch(96% 0.014 110)", minHeight: "calc(100vh - 120px)" }}>
        <div className="container">
          ...
          {showForm && (
            <div className="card-vintage p-6 mb-6">
              ...
              <label className="flex items-center gap-2 mb-4 cursor-pointer">
                <input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} style={{ accentColor: "oklch(33.1% 0.064 144.7)" }} />
                <span style={{ fontFamily: "'EB Garamond', serif", fontSize: "0.9rem", color: "oklch(42.3% 0.087 144.3)" }}>Active (visible on homepage)</span>
              </label>
          ...
          {isLoading ? <div className="text-center py-12" style={{ fontFamily: "'EB Garamond', serif", color: "oklch(64.3% 0.161 143.4)" }}>Loading...</div> : slides && slides.length > 0 ? (
              ...
              {slides.map((slide: any) => (
                <div key={slide.id} className="card-vintage p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p style={{ fontFamily: "'EB Garamond', serif", fontSize: "0.8rem", color: "oklch(64.3% 0.161 143.4)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{slide.subtitle || slide.imageUrl}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateSlide.mutate({ id: slide.id, active: !slide.active })} style={{ background: "none", border: "none", cursor: "pointer", color: slide.active ? "oklch(42.3% 0.087 144.3)" : "oklch(64.3% 0.161 143.4)" }} title={slide.active ? "Hide slide" : "Show slide"}>
            ...
            <div className="text-center py-16">
              <p style={{ fontFamily: "'EB Garamond', serif", color: "oklch(64.3% 0.161 143.4)", fontStyle: "italic" }}>No slides yet. Add your first hero image.</p>
            </div>
          )}
        </div>
      </div>
